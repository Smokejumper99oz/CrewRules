...
function formatFoundingRole(role: string | null | undefined): string {
          ...
          <tbody>
            {members.map((m) => (
              <tr key={m.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="px-4 py-3 text-slate-300">{getTenantAirlineDisplayName(m.tenant)}</td>
                <td className="px-4 py-3 text-slate-300">{formatFoundingRole(m.role)}</td>
