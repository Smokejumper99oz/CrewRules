...
export default function LibraryPage() {
  ...
  async function load() {
    setLoading(true);
    setError(null);
    const [admin, { docs: list, error: err }] = await Promise.all([getIsAdmin(), listDocuments()]);
    setIsAdmin(admin);
    setDocs(list);
    if (err) setError(err);
    setLoading(false);
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
      ...
      <p className="mt-2 text-slate-300">
        Documents available for Ask AI and download. Admins can manage files.
      </p>
      ...
      {!loading && docs.length === 0 && !error && (
        <p className="mt-6 text-slate-400">
          No documents yet. Admins can upload in{" "}
