...
const SAMPLE_ANSWER = "AI search over your CBA and uploaded documents is coming next. Your question will be answered with citations to specific sections and pages.";
...
export default function AskPage() {
  ...
  const displayAnswer = submittedQuestion
    ? "AI search over your CBA and documents is coming next. Your question will be answered with citations to specific contract sections and pages."
    : SAMPLE_ANSWER;
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-6">
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:gap-2">
          <input
            ...
            className="min-h-[44px] flex-1 rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3 text-white placeholder:text-slate-600 outline-none focus:border-emerald-400/40 touch-manipulation"
          />
          <button
            type="submit"
            className="min-h-[44px] shrink-0 rounded-xl bg-[#75C043] px-5 py-3 font-semibold text-slate-950 hover:opacity-95 transition touch-manipulation"
          >
            Ask
          </button>
        </div>
      ...
      {/* Result card - matches landing page mock-up */}
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-3 rounded-xl bg-emerald-500/10 ring-1 ring-emerald-500/20 p-3">
              <div className="text-xs text-emerald-200">
                {submittedQuestion ? "Citation will appear here once AI search is connected." : SAMPLE_REFERENCE}
              </div>
            </div>
          </div>
