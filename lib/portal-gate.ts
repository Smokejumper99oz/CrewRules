...
export async function gateUserForPortal(
  ...
  const { data: minimalProfile } = await supabase
    .from("profiles")
    .select("id, role, tenant, portal, email, subscription_tier, pro_trial_started_at, pro_trial_expires_at, is_founding_pilot, welcome_modal_version_seen, color_mode")
    .eq("id", user.id)
    .maybeSingle();
  ...
  const { data: profile, error } = await supabase
    .from("profiles")
    .select("id, role, tenant, portal, email, subscription_tier, pro_trial_started_at, pro_trial_expires_at, is_founding_pilot, welcome_modal_version_seen, base_airport, position, date_of_hire, home_airport, color_mode")
    .eq("id", user.id)
    .eq("tenant", tenant)
    .eq("portal", portal)
    .single();
