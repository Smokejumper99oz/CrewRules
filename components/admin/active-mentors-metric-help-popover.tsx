...
export function ActiveMentorsMetricHelpPopover() {
  ...
  const updatePosition = useCallback(() => {
    ...
    // Right-align under the ⓘ control, then clamp so the full panel stays in the viewport
  ...
  const panelNode =
    ...
    typeof document !== "undefined" ? (
      <div
        ref={panelRef}
        id="active-mentors-metric-help-panel"
        role="dialog"
        aria-labelledby="active-mentors-metric-help-title"
        style={{
          position: "fixed",
          top: panelBox.top,
          left: panelBox.left,
      ...
      >
        <h3 id="active-mentors-metric-help-title" className="text-sm font-semibold text-slate-900">
          How this metric works
        </h3>
        <div className="mt-3 space-y-2.5 text-sm leading-relaxed text-slate-600">
          <p>The large number shows your current active mentors.</p>
          <p>The change below shows this month&apos;s roster movement.</p>
          <div>
            <p className="font-medium text-slate-800">This month&apos;s activity:</p>
            <ul className="mt-1.5 list-disc space-y-1 pl-5">
              <li>Added: mentors onboarded to the program</li>
              <li>Inactive: mentors set to inactive, former, or archived</li>
            </ul>
          </div>
          <p className="font-medium text-slate-800">Net change = Added − Inactive</p>
          <p className="border-t border-slate-200 pt-3 text-xs leading-relaxed text-slate-500">
            Note: &ldquo;Inactive&rdquo; reflects status updates this month and may include administrative
            updates.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="shrink-0">
      <button
        ...
        aria-controls="active-mentors-metric-help-panel"
